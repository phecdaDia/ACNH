...
for filename, row_class in specs.lookup.items():
	...
	csvw = csv.writer(open('%s/%s.csv' % (csv_dir, filename.replace('.bcsv', '')), 'w', encoding='utf-16le'))
	...
	with open('%s/%s.html' % (html_dir, filename.replace('.bcsv', '')), 'w', encoding='utf-16le') as f:
		for line in html:
			f.write(line)
			f.write('\n')
	with open('%s/%s.json' % (json_dir, filename.replace('.bcsv', '')), 'w', encoding='utf-16le') as f:
